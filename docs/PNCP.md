...
O coletor passa a executar a cada 15 minutos. As modalidades pendentes são retomadas em todas as rodadas; modalidades concluídas aguardam uma hora para nova janela. Até 25 páginas/2 minutos são reservados para iniciar consultas, e o restante atende os detalhes pendentes. O lote de detalhes passa de um teto de 50 por hora para até 150 por execução, limitado a 8 minutos de início de novas oportunidades. Chamadas em andamento e retries podem prolongar a rodada; o Railway não inicia outra execução do mesmo cron enquanto a anterior estiver ativa.
...
## Coleta distribuída e métricas (16/09/2026)
Cada execução de 15 minutos retoma as modalidades pendentes, com até 25 páginas, 3 por modalidade e orçamento de 2 minutos para iniciar consultas. Modalidades concluídas há menos de uma hora são puladas sem chamada externa. A ordem de última atualização dá vez às modalidades ainda não atendidas.

No cron, cada requisição tem uma tentativa por rodada: falhas são adiadas no banco, evitando gastar toda a rodada em retries imediatos. Timeout de 45 segundos, intervalo de 2 segundos e Retry-After/cooldown permanecem. O restante do lote (8 minutos para iniciar oportunidades) completa detalhes; uma oportunidade já iniciada pode ultrapassar esse orçamento. O cliente interativo conserva até três tentativas.
