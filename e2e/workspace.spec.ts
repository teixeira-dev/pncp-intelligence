...
const opportunityId="11222333000181-1-"+String(parseInt(suffix.slice(0,6),16))+"/2026";
...
test.beforeAll(async()=>{
 ...
 await upsertOpportunity({numeroControlePNCP:opportunityId,anoCompra:2026,sequencialCompra:1,numeroCompra:suffix,objetoCompra:"Material hospitalar "+suffix,modalidadeId:6,modalidadeNome:"Pregão eletrônico",situacaoCompraNome:"Divulgada no PNCP",dataPublicacaoPncp:new Date().toISOString(),orgaoEntidade:{cnpj:"11222333000181",razaoSocial:"Órgão de teste"},unidadeOrgao:{municipioNome:"Recife",ufSigla:"PE"}});

});
test.afterAll(async()=>{await db.opportunity.deleteMany({where:{id:opportunityId}});if(userId)await db.user.delete({where:{id:userId}});if(organizationId)await db.organization.delete({where:{id:organizationId}});await db.$disconnect();});
