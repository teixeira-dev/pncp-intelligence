...
async function main(){
 ...
 console.info(JSON.stringify({event:"SYNC_PROCESS_STARTED",opportunities:await db.opportunity.count(),previous:previous?{...previous,error:previous.error?syncError(new Error(previous.error)):null}:null}));
 await runSync();
