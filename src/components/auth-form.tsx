...
export function AuthForm({mode}:{mode:"login"|"recover"|"reset"}){
 ...
 try{const r=await fetch("/api/auth/"+mode,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(fields)});const d=await r.json();if(!r.ok)throw new Error(d.error);if(mode==="login")router.replace("/dashboard");else if(mode==="reset")router.replace("/login");else setMessage(d.message);}catch(e){setMessage(e instanceof Error?e.message:"Falha de conexão.");}finally{setBusy(false);}
 }
