...
export async function enrichOpportunity(id:string,options:{fetcher?:typeof fetch;sleep?:typeof wait}={}){
 ...
 await sleep(2000);
 const result=await fetchJson(new URL(`${base}/${kind}?pagina=${page}&tamanhoPagina=50`),options.fetcher,sleep);
...
export async function importPendingDetails(options:{limit?:number;newest?:boolean;budgetMs?:number;fetcher?:typeof fetch;sleep?:typeof wait}={}){
 ...
 if(error instanceof PNCPError&&(error.status===429||error.retryAt)){await db.syncCursor.upsert({where:{id:"PNCP_COOLDOWN"},create:{id:"PNCP_COOLDOWN",through:retryAt},update:{through:retryAt}});break;}
 }
 }
 const pending=await db.$queryRaw<{exists:boolean}[]>`SELECT EXISTS(SELECT 1 FROM "Opportunity" WHERE "detailsHash" IS NULL OR "detailsHash" <> "contentHash") AS exists`;
 return {imported,pending:pending[0].exists};
}
